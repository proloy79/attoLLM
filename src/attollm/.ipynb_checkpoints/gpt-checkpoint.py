...
from attollm.embedding_block import sinusoidal_positions
...
class GPTConfig:
    ...
    - pos_type: 'learned' (GPT‑style) or 'sinusoidal'
...
class GPT(nn.Module):
    ...
    def __init__(self, cfg: GPTConfig):
        ...
        # cfg.n_layers of Transformer blocks
    ...
    def forward(
        ...
        if self.cfg.pos_type == "learned":
            ...
            pe = sinusoidal_positions( T, self.cfg.d_model)  # [T, D]
            x = x + pe[None, :, :]  # [B, T, D]
        ...
        # Final norm and LM head projection applied on the final output from the tranforms
        #print('post transform', x.shape)
        x = self.norm_f(x)
        #print('post transform norm', x.shape)
